...
const Watch = () => {
  ...
  useEffect(() => {
    if (
      localStorage.getItem("StagedStreamEmbedMode") !== undefined &&
      localStorage.getItem("StagedStreamEmbedMode") !== null
    )
      setEmbedMode(
        JSON.parse(localStorage.getItem("StagedStreamEmbedMode") || "false"),
      );
    else setEmbedMode(false);
    const latestAgg: any = localStorage.getItem("StagedStreamLatestAgg");
  ...
  useEffect(() => {
    if (embedMode !== undefined && embedMode !== null)
      localStorage.setItem("StagedStreamEmbedMode", embedMode);
    if (embedMode === true) {
      const latestAgg: any = localStorage.getItem("StagedStreamLatestAgg");
      if (latestAgg !== null && latestAgg !== undefined) setSource(latestAgg);
      toast.info(
      ...
      <div className={styles.watchSelects}>
        {embedMode === true && (
          <select
            ...
            onChange={(e) => {
              ...
              localStorage.setItem("StagedStreamLatestAgg", e.target.value);
            }}
          >
        ...
        <select
          ...
          onChange={(e) => {
            ...
            localStorage.setItem("StagedStreamEmbedMode", e.target.value);
          }}
        >
